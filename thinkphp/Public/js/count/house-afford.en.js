...
PMSFLWrite("\n");
PMSFLWrite("</style>\n");
PMSFLWrite("<div id=\"pmsfl-calculator-house-afford\" class=\"pmsfl-calculator\">\n");
PMSFLWrite("<div class=\"calc-outer\">\n");
PMSFLWrite("<div class=\"calc-inner\">\n");
PMSFLWrite("<h1>您可以支付什么价位的房子？</h1>\n");
PMSFLWrite("\n");
PMSFLWrite("<p>\n");
PMSFLWrite("房产会给家庭未来的财务状况带来重要影响。购房前，我们建议您算一算自己能够买得起什么价位的房子？\n");
PMSFLWrite("</p>\n");
PMSFLWrite("\n");
PMSFLWrite("<form>\n");
PMSFLWrite("<table>\n");
...
PMSFLWrite("<td class=\"calc-col-3of4\"><input name=\"down\" type=\"text\" size=\"10\"/></td>\n");
PMSFLWrite("<td class=\"calc-col-4of4\"></td>\n");
PMSFLWrite("</tr>\n");
PMSFLWrite("\n");
PMSFLWrite("<tr>\n");
PMSFLWrite("<td>您希望在多长时间内还清房贷？</td>\n");
PMSFLWrite("<td>&#160;</td>\n");
PMSFLWrite("<td>\n");
PMSFLWrite("<select name=\"mortgage\">\n");
PMSFLWrite("<option value=\"60\">5年</option>\n");
PMSFLWrite("<option value=\"120\">10年</option>\n");
PMSFLWrite("<option value=\"180\">15年</option>\n");
PMSFLWrite("<option value=\"240\">20年</option>\n");
PMSFLWrite("<option value=\"300\">25年</option>\n");
PMSFLWrite("<option value=\"360\">30年</option>\n");
PMSFLWrite("</select>\n");
PMSFLWrite("</td>\n");
PMSFLWrite("<td>&#160;</td>\n");
PMSFLWrite("</tr>\n");
PMSFLWrite("\n");
PMSFLWrite("<tr>\n");
PMSFLWrite("<td>您每月能支付多少房贷？</td>\n");
PMSFLWrite("<td>￥</td>\n");
PMSFLWrite("<td><input name=\"payment\" type=\"text\" size=\"10\"/></td>\n");
PMSFLWrite("<td>&#160;</td>\n");
PMSFLWrite("</tr>\n");
PMSFLWrite("\n");
PMSFLWrite("<tr>\n");
PMSFLWrite("<td>您能申请到多少的房贷利率？</td>\n");
PMSFLWrite("<td>&#160;</td>\n");
...
PMSFLWrite("\n");
PMSFLWrite("<tr>\n");
PMSFLWrite("<td>预计产权交易费用</td>\n");
PMSFLWrite("<td>￥</td>\n");
PMSFLWrite("<td><input name=\"misc\" type=\"text\" size=\"10\"/></td>\n");
PMSFLWrite("<td>&#160;</td>\n");
PMSFLWrite("</tr>\n");
PMSFLWrite("\n");
PMSFLWrite("<tr>\n");
PMSFLWrite("<td>手续费</td>\n");
PMSFLWrite("<td>&#160;</td>\n");
PMSFLWrite("<td><input name=\"origination\" type=\"text\" size=\"10\"/></td>\n");
PMSFLWrite("<td>%</td>\n");
PMSFLWrite("</tr>\n");
PMSFLWrite("\n");
PMSFLWrite("<tr>\n");
PMSFLWrite("<td>不动产税</td>\n");
PMSFLWrite("<td>&#160;</td>\n");
PMSFLWrite("<td><input name=\"tax\" type=\"text\" size=\"10\"/></td>\n");
PMSFLWrite("<td>%</td>\n");
PMSFLWrite("</tr>\n");
PMSFLWrite("\n");
PMSFLWrite("<tr>\n");
PMSFLWrite("<td>房屋保险费率</td>\n");
PMSFLWrite("<td>&#160;</td>\n");
PMSFLWrite("<td><input name=\"insurance\" type=\"text\" size=\"10\"/></td>\n");
PMSFLWrite("<td>%</td>\n");
PMSFLWrite("</tr>\n");
PMSFLWrite("\n");
PMSFLWrite("<tr>\n");
...
PMSFLWrite("<div id=\"pmsfl-calculator-house-afford-err1\">请输入一个有效的金额</div>\n");
PMSFLWrite("<div id=\"pmsfl-calculator-house-afford-err2\">请输入一个有效的金额</div>\n");
PMSFLWrite("<div id=\"pmsfl-calculator-house-afford-err3\">根据您输入的数值，您偿还债务时会遇到困难。请您输入合适的数值再进行计算</div>\n");
PMSFLWrite("</div>\n");
PMSFLWrite("\n");
...
PMSFLWrite("\n");
PMSFLWrite("<p>\n");
PMSFLWrite("您大概可以购买一个价值<span id=\"pmsfl-calculator-house-afford-price\"></span> 元的房子。\n");
PMSFLWrite("</p>\n");
PMSFLWrite("\n");
PMSFLWrite("</div>\n");
PMSFLWrite("\n");
...
PmsflCalc('house-afford', {
  calculate: function (helper) {
    var p  = helper.parseFloat('payment');
    var n  = helper.parseInt('mortgage');
    var i  = helper.parseFloat('interest');
    var ir = helper.parseFloat('insurance', '0.0');
    var tr = helper.parseFloat('tax', '0.0');
    var down = helper.parseFloat('down');
    var orig_per = helper.parseFloat('origination', '0.0');
    var misc_closing_costs = helper.parseFloat('misc', '0.0');

    if (isNaN(down)) {return helper.alert(1, 'down');}
    if (isNaN(p))  {return helper.alert(1, 'payment');}
    if (isNaN(i))  {return helper.alert(2, 'interest');}
    if (isNaN(misc_closing_costs)) {return helper.alert(1, 'misc');}
    if ((down - misc_closing_costs) <= 0) {return helper.alert(3, 'down');}
    if (isNaN(orig_per)) {return helper.alert(2, 'origination');}
    if (isNaN(tr)) {return helper.alert(2, 'tax');}
    if (isNaN(ir)) {return helper.alert(2, 'insurance');}

    
    i  = i  / 100.0 / 12.0;
    ir = ir / 100.0 / 12.0;
    tr = tr / 100.0 / 12.0;
    orig_per = orig_per / 100.0;
    
    var afford = 0.0;
    var x      = 0.0;
    var tmp    = 0.0;

    if (i > 0) {tmp = (1.0/i)*(1.0 - Math.pow((1.0/(1.0 + i)),n));}
    x = tmp * (1.0 - orig_per);
    afford = (p * x + down - misc_closing_costs) / (1.0 + (ir + tr) * x);
    if (afford < 0.0) {afford = 0.0;}
    
    helper.set('price', afford.toFixed(2));
    helper.show();
  }
});
