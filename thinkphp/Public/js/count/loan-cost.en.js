...
PMSFLWrite("width:446px !important;\n");
PMSFLWrite("/*border: #00CC00 thin solid !important;*/\n");
PMSFLWrite("}\n");
PMSFLWrite("\n");
...
PMSFLWrite("\n");
PMSFLWrite("</style>\n");
PMSFLWrite("<div id=\"pmsfl-calculator-loan-cost\" class=\"pmsfl-calculator\">\n");
PMSFLWrite("<div class=\"calc-outer\">\n");
PMSFLWrite("<div class=\"calc-inner\">\n");
PMSFLWrite("<h1>贷款需要多少成本？</h1>\n");
PMSFLWrite("\n");
PMSFLWrite("<p>\n");
PMSFLWrite("每一笔贷款都由相应的利息支付。您贷款的价值能否抵得上支付的利息呢？算一算吧！\n");
PMSFLWrite("</p>\n");
PMSFLWrite("\n");
PMSFLWrite("<form>\n");
PMSFLWrite("<table>\n");
PMSFLWrite("\n");
PMSFLWrite("<tr>\n");
PMSFLWrite("<td class=\"calc-col-1of4\">贷款金额</td>\n");
PMSFLWrite("<td class=\"calc-col-2of4\">￥</td>\n");
PMSFLWrite("<td class=\"calc-col-3of4\"><input name=\"tla\" type=\"text\" value=\"\" size=\"10\"/></td>\n");
PMSFLWrite("<td class=\"calc-col-4of4\">&#160;</td>\n");
PMSFLWrite("</tr>\n");
PMSFLWrite("\n");
PMSFLWrite("<tr>\n");
PMSFLWrite("<td>贷款年利率</td>\n");
PMSFLWrite("<td>&#160;</td>\n");
PMSFLWrite("<td><input name=\"apr\" type=\"text\"  value=\"\" size=\"10\"/></td>\n");
PMSFLWrite("<td>%</td>\n");
PMSFLWrite("</tr>\n");
PMSFLWrite("\n");
PMSFLWrite("<tr>\n");
PMSFLWrite("<td colspan=\"4\">\n");
PMSFLWrite("<h3>请输入下面两项中的一项：</h3>\n");
PMSFLWrite("</td>\n");
PMSFLWrite("</tr>\n");
PMSFLWrite("\n");
PMSFLWrite("<tr>\n");
PMSFLWrite("<td>每月还款金额</td>\n");
PMSFLWrite("<td>￥</td>\n");
PMSFLWrite("<td><input name=\"mpay\" type=\"text\" value=\"\" size=\"10\"/></td>\n");
PMSFLWrite("<td>&#160;</td>\n");
PMSFLWrite("</tr>\n");
PMSFLWrite("\n");
PMSFLWrite("<tr>\n");
PMSFLWrite("<td>贷款期限</td>\n");
PMSFLWrite("<td>&#160;</td>\n");
PMSFLWrite("<td><input name=\"months\" type=\"text\" value=\"\" size=\"10\"/></td>\n");
PMSFLWrite("<td>月</td>\n");
PMSFLWrite("</tr>\n");
PMSFLWrite("\n");
PMSFLWrite("<tr>\n");
...
PMSFLWrite("<div id=\"pmsfl-calculator-loan-cost-err1\">请您输入有效的数值再进行计算</div>\n");
PMSFLWrite("<div id=\"pmsfl-calculator-loan-cost-err2\">请您输入有效的数值再进行计算</div>\n");
PMSFLWrite("<div id=\"pmsfl-calculator-loan-cost-err3\">根据您输入的数值，您偿还债务时会遇到困难。请您输入合适的数值再进行计算</div>\n");
PMSFLWrite("<div id=\"pmsfl-calculator-loan-cost-err4\">只能输入月还款金额和贷款期限中的一项</div>\n");
PMSFLWrite("</div>\n");
PMSFLWrite("\n");
...
PMSFLWrite("<table>\n");
PMSFLWrite("<tr>\n");
PMSFLWrite("<td>最小每月还款</td>\n");
PMSFLWrite("<td>￥</td>\n");
PMSFLWrite("<td><span id=\"pmsfl-calculator-loan-cost-mpay\"></span></td>\n");
PMSFLWrite("</tr>\n");
PMSFLWrite("<tr>\n");
PMSFLWrite("<td>贷款期限</td>\n");
PMSFLWrite("<td>&#160;</td>\n");
PMSFLWrite("<td><span id=\"pmsfl-calculator-loan-cost-nummon\"></span>月</td>\n");
PMSFLWrite("</tr>\n");
PMSFLWrite("<tr>\n");
PMSFLWrite("<td>总利息</td>\n");
PMSFLWrite("<td>￥</td>\n");
PMSFLWrite("<td><span id=\"pmsfl-calculator-loan-cost-fcharge\"></span></td>\n");
PMSFLWrite("</tr>\n");
PMSFLWrite("<tr>\n");
PMSFLWrite("<td>总还款额（本金+利息）</td>\n");
PMSFLWrite("<td>￥</td>\n");
PMSFLWrite("<td><span id=\"pmsfl-calculator-loan-cost-tcharge\"></span></td>\n");
PMSFLWrite("</tr>\n");
PMSFLWrite("</table>\n");
PMSFLWrite("</div>\n");
PMSFLWrite("</div>\n");
...
PmsflCalc('loan-cost', function (helper) {
  var tla    = helper.parseFloat('tla');
  var apr    = helper.parseFloat('apr');
  var mpay   = helper.parseFloat('mpay', '0.0');
  var months = helper.parseInt('months', '0');

  if (isNaN(tla))    {return helper.alert(1, 'tla');}
  if (isNaN(apr))    {return helper.alert(2, 'apr');}
  if (isNaN(mpay))   {return helper.alert(4, 'mpay');}
  if (isNaN(months)) {return helper.alert(4, 'months');}
  
  if (mpay > 0 && months > 0) {
    return helper.alert(4, 'mpay');
  } else if (mpay <= 0 && months <= 0) {
    return helper.alert(4, 'months');
  }
  
  mr = apr/1200.0;
  min_pay = tla/36.0;
  first_interest_pmt = mr * tla;
  ...
  if (mpay > 0.0) {
    if ((mr * tla) >= mpay) {return helper.alert(3, 'mpay');}
    ...
    if (mr > 0) {
      ...
      totcost = (nummon - 1.0) * mpay;
      fincharg = totcost - tla;
    } else {
      if ((tla % mpay) == 0) {
        nummon = tla / mpay;
        finalpay = mpay;
        ...
        nummon = tla / mpay + 1.0;
        finalpay = (nummon - Math.floor(nummon)) * mp;
    ...
    if (mr > 0) {
      tmp1 = (1.0 / mr) * (1.0 - (1.0 / Math.pow((1.0 + mr), months)));
      mpay = tla / tmp1;
      totcost = mpay * months;
      fincharg = totcost - tla;
    } else {
  ...
  helper.set('mpay', mpay.toFixed(2));
  helper.set('nummon', Math.floor(nummon));
  helper.set('fcharge', fincharg.toFixed(2));
  helper.set('tcharge', totcost.toFixed(2));
  helper.show();
});
