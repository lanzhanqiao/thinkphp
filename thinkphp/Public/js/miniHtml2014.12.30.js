...
Calculator.prototype.calculate = function(principal, period, rate,flag){
    ...
    var preFeeRate = this._getPreFeeRate(period,flag) //前期服务费率
        , contractAmount = this._getContractAmount(principal,preFeeRate) //合同金额
        , mr = rate/12 //月利率
...
Calculator.prototype._getContractAmount = function(principal, preFeeRate){
	var contractAmount = give(principal/(1-preFeeRate));
	var inHandAmt = give((contractAmount - 100) * (1-preFeeRate)) + 100;
	if(principal == inHandAmt){
		contractAmount -= 100;
	}
