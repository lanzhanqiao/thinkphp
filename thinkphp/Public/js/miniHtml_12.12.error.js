...
Calculator.prototype.INTEREST_RATE_DICT = {12: 0.1, 18: 0.105, 24: 0.11, 36: 0.12, 48: 0.125};
Calculator.prototype.M_FEE_RATE_DICT = {12:0.0033, 18:0.00307, 24:0.00284, 36:0.00235, 48:0.0021};
Calculator.prototype.PRE_FEE_RATE_DICT = {12:0.0521, 18:0.0745, 24:0.0938, 36:0.1256, 48:0.1535};
Calculator.prototype.calculate = function(principal, period, rate){
    if(!/^\d+$/.test(principal)){
        throw "principal value error!";
    }
...
Calculator.prototype.calPrincipal = function (amount, period) {
    if(!/^\d+$/.test(amount) || amount <= 0){
        throw "release amount value error!";
    }
    if((!/^\d+$/.test(period))||(period*10)%10!=0||period<1||period>48){
        throw "period value error!"
    }
    return Math.floor((amount / (1 - Calculator.prototype.PRE_FEE_RATE_DICT[period])) / 100) * 100;
};
Calculator.prototype._monthPay = function(principal, period, mr){
    var v1 = Math.pow(mr+1, period), v2 = principal * v1 * mr, v3 = v1- 1, result = v2/v3;
    return result;
...
Calculator.prototype._genfare = function(plan){
    var rate = Calculator.prototype.M_FEE_RATE_DICT[plan.period];
    switch(plan.mode){
        case this.JY:
            plan.mfare = round(rate * plan.principal);
            plan.fare = plan.mfare * plan.period;
            ...
            break;
        case this.JY_129:
            var preFee = plan.principal * Calculator.prototype.PRE_FEE_RATE_DICT[plan.period];
            plan.mfare = round(rate * plan.principal);
            plan.loan = Math.ceil((plan.principal - preFee) / 100) * 100;
            plan.prefee = plan.principal - plan.loan;
            plan.fare = plan.mfare * plan.period + plan.prefee;
            plan.balance = round((plan.total + plan.fare - plan.prefee));
            break;
        case this.NORMAL:
        default :
            plan.fare = ((plan.period)>2?(plan.principal)*(0.04+((plan.period)-2)*0.002):(plan.principal)*0.04);
...
Calculator.prototype._genPlan = function(plan){
    var pay_unit = [], p_balance = plan.principal, i_balance = plan.interest;
