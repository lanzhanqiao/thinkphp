...
$(function(){
    ...
    $(".calculatorBtn").click(function (){ 
        ...
        if(mode=="jy"){
			...
			if(parseInt(principal)>10000 && principal < parseInt(500000) && parseInt(principal)%100 >0){
				alert("请输入100的倍数，且在10,000，到500,000之间");
			}
        }
		
		if(mode =="lender"){
			var plan = getCal("lender").calculate( parseFloat(principal), parseFloat(period), parseFloat(rate/100));
		}else{
			if(qualify =="078"){
				rate = INTEREST_RATE_DICT[period];
            	var plan = getCal("078").calculate( parseFloat(principal), parseFloat(period), parseFloat(rate));
			}else if(qualify == "189"){
				rate = INTEREST_RATE_DICT[period];
				var plan = getCal("189").calculate( parseFloat(principal), parseFloat(period), parseFloat(rate));
			}
		}
