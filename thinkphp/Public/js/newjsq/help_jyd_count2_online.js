    ...
    $(".calculatorBtn").click(function (){ 
        ...
        if(mode=="jy"){
		   if(parseInt(principal) <=0 ||parseInt(principal) <20000 || parseInt(principal)>500000 ){
			...
			if(parseInt(principal)>20000 && principal < parseInt(500000) && parseInt(principal)%100 >0){
				alert("请输入100的倍数，且在20,000，到500,000之间");
				$("#principal").val("");
				return;
			}
        }
		...
		if(mode =="lender"){
			...
			if(qualify =="078"){
            	...
            	var plan = getCal("078").calculate( parseFloat(principal), parseFloat(period), parseFloat(rate),"line");
                $(".agreement").attr("href","http://www.yirendai.com/entry/agreement/borrow_agreement_special.html?pr="+plan.principal+"&pe="+plan.period+"&ra="+plan.rate+"&mp="+plan.month_pay+"&qualify=078"+"&contractAmount="+plan.contractAmount).show();
			}else if(qualify == "189"){
				rate = INTEREST_RATE_DICT[period];
				var plan = getCal("189").calculate( parseFloat(principal), parseFloat(period), parseFloat(rate),"line");
                $(".agreement").attr("href","http://www.yirendai.com/entry/agreement/borrow_agreement_special.html?pr="+plan.principal+"&pe="+plan.period+"&ra="+plan.rate+"&mp="+plan.month_pay+"&qualify=189"+"&contractAmount="+plan.contractAmount).show();
			}
		}
