		...
		success:function(json){
			...
			if(ydy_status){
				...
				for(var i=0,j=0,k=0;i<total_record;i++){
					...
					//console.log(jiejiegao_list+"::"+onlineStatus+"::"+publishStatus);
				}
				...
				if(has_jiejiegao){
					...
					jiejiegao += '<div class="yao_ydy_new_navLeftjj clearfix"><h2>节节高</h2><span></span><p>收益率逐月递增</p><a href="'+yrd_url+'">更多 ›</a></div>';
					...
					if(jiejiegao_list[0].onlineStatus=='0'){
						jiejiegao += '<ul class="main_one_buttonjj"><li><a href="'+yrd_url+'"></a></li></ul>';
					}
					else if(jiejiegao_list[0].onlineStatus=='1'){
			...
			$.each(items,function (key,ele){
				...
				//拼接新手标的信息
				html_str +='<div class="new_bd_ad"><a href="/specialpage/yqzpc/"><img src="/ccms/images/newyrd/index_ads.jpg"></a></div>' ;
